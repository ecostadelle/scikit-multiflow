...
def test_check_weights():
    ...
    weights = check_weights(np.float_(test_value))
    assert weights == np.float_(test_value)

    weights = check_weights(np.int(test_value))
    assert weights == np.int(test_value)

    weights = check_weights(np.array([test_value], np.float_))
    assert isinstance(weights, np.ndarray)
    assert isinstance(weights[0], np.float_)
